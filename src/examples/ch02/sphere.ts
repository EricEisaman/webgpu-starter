...
const createPipeline = async (
  ...
  // uniform bind group
  const uniformBindGroup = ws.createBindGroup(
    init.device,
    pipeline.getBindGroupLayout(0),
    [uniformBuffer, colorUniformBuffer]
  );
  const uniformBindGroup2 = ws.createBindGroup(
    init.device,
...
const run = async () => {
  ...
  const frame = () => {
    ...
    init.device.queue.writeBuffer(
      p.uniformBuffers[0],
      0,
      mvpMat as ArrayBuffer
    );
    init.device.queue.writeBuffer(
      p.uniformBuffers[1],
      0,
      ws.hex2rgb(params.objectColor)
    );
    init.device.queue.writeBuffer(
      p.uniformBuffers[2],
      0,
      ws.hex2rgb(params.wireframeColor)
    );
